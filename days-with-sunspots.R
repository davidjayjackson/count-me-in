...
sidc1$Vote <- ifelse(sidc$Spots ==0,0,1)
Fit <- as.data.frame(lowess(sidc1$Spots,f=.2))
sidc1 <-cbind(sidc1,Fit$y)
colnames(sidc1) <-c("Ymd","Spots","Loess")
##
## Current Solar Miniumn: 2014 - Oct. 2019
##
sidc2 <-  sidc2 &>% filter(Ymd >="2014-01-01") 
ggplot(data=sidc2,aes(x=Ymd,y=Spots)) + geom_line() + geom_line(data=sdic2,aes(x=Ymd,y=Loess))
## Create monthly summary field with XTS
